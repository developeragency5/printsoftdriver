...
const hubAdditions = [
  ['network-drivers.html', TOPICS.filter(t => ['fix-no-wifi-after-update', 'fix-bluetooth-pairing-fails'].includes(t.slug)), 'More Network Driver Reads'],
  ...
  ['usb-drivers.html', TOPICS.filter(t => ['extract-driver-from-installer'].includes(t.slug)), 'More Controller Reads'],
