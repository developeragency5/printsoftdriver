...
A pure HTML/CSS/Vanilla JavaScript multi-page informational website about device drivers. **135 pages total**.
...
- Apr 2026 — removed `refund-policy.html` and unlinked it from the footer Legal column and the footer bottom-bar on all 134 remaining pages. The page had been added briefly in an earlier round to satisfy an AdScan compliance check, but the Site sells nothing, so a refund policy was logically inconsistent. Page count back to 134.
- Apr 2026 — closed two new audit notices: (1) "Page has only one incoming internal link" against `fix-network-device-offline.html`, which was previously linked only from `sitemap.html` — added in-content `<a href="fix-network-device-offline.html">` mentions inside the relevant body sections of `network-drivers.html` (the topical hub), `knowledge.html` (Wi-Fi symptom card), `fix-wifi-drops.html`, and `fix-no-wifi-after-update.html`, taking the inbound count from 1 to 5. Used real anchor text inside body paragraphs (not footer/nav links) since auditors discount boilerplate cross-links. (2) "Content not optimized" against `disclaimer.html` — root cause was the JSON-LD `BreadcrumbList` referencing `https://www.printsoftdriver.com/legal-hub.html`, which 404s; the on-page breadcrumb correctly points to the existing `privacy.html` legal hub. Synced the JSON-LD breadcrumb URL to `privacy.html` so structured data matches reality.
- May 2026 — added two US-website-compliance items requested by user: (1) created `accessibility-statement.html`, a full WCAG 2.1 AA accessibility statement modelled on `disclaimer.html`'s structure (Quick Answer + In Brief + Table of Contents + 9 body sections including Commitment, Conformance Standard, Measures Taken, Accessibility Features, Known Limitations, Third-Party Content, Feedback, Formal Enforcement, Self-Assessment Approach + 5-question FAQ + 3 JSON-LD blocks WebPage/Article/FAQPage). Title 60 chars, meta description 142 chars, all metadata fields synced. The new page is linked from the footer Legal column on **all 135 pages** (script-driven sweep) and added to the legal-crosslinks-grid on all 4 other legal pages and to the body listing of `sitemap.html`. (2) Expanded `privacy-policy.html` with multi-state coverage: renamed the CCPA section to "CCPA / CPRA" and added the CPRA-specific rights (correction, sensitive data limit, opt-out of sharing for cross-context behavioural advertising); added a new "Your Rights Under Other US State Privacy Laws" section listing VCDPA/CPA/CTDPA/UCPA/TDPSA/OCPA plus Montana/Tennessee/Iowa/Indiana/Delaware/New Hampshire/New Jersey/Minnesota/Maryland with a consolidated rights paragraph; added a new "Sensitive Personal Information" section confirming we do not knowingly collect any. The "Sale and sharing" subsection clarifies we do not sell personal info and do not share for cross-context behavioural advertising. Page count: **134 → 135** (added accessibility-statement.html). Strict no-brands rule still verified clean (0 hits for microsoft / bing / google).
...
- Footer Legal column links to: Privacy Policy, Terms of Use, Disclaimer, Cookie Policy
- No "Refund Policy" anywhere on the Site — the Site sells nothing, so a refund policy would be nonsensical (it was added briefly in an earlier round to satisfy an AdScan check, then removed at user instruction)
