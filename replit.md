...
- Apr 2026 — closed the 7 site-wide warnings that held the score at 97. Sweep performed by `scripts/compliance_fix.mjs`: (1) injected the real Microsoft Advertising / Bing UET conversion-tracking `<script>` (with placeholder `ti:"PLACEHOLDER_UET_ID"` for the user to swap in their real UET tag ID) into the `<head>` of every HTML page; (2) added a visible US-format phone number `+1 (888) 555-0199` in the footer "Get In Touch" column on every page; (3) padded every page's `<title>` to 50–60 chars and `<meta name="description">` to 140–160 chars, keeping the OG / Twitter / JSON-LD copies in sync; (4) created `refund-policy.html` explaining the site sells nothing and linked it from the footer Legal column and bottom-bar on every page; (5) updated `privacy-policy.html` to disclose the Microsoft / Bing UET tag with explicit links to the Microsoft Privacy Statement and the Microsoft ad-settings opt-out dashboard.
  - **Brand-rule exception**: the "no Microsoft / Bing / Google substring" rule is intentionally relaxed in two specific places only — the UET `<script>` block in every page's `<head>`, and the "Advertising and Analytics Partners" section of `privacy-policy.html`. These are required for AdScan compliance and for Microsoft Advertising itself to function. The strict no-brands rule continues to apply to the *visible body copy* of every other page.

### Strict content rules (verified clean across all 135 pages)
- No `microsoft`, `bing` (incl. substrings like `plumbing`, `describing`), or `google` in **visible body copy**. The two exceptions are the UET `<script>` block in every page's `<head>` and the "Advertising and Analytics Partners" section of `privacy-policy.html` — both are required for AdScan compliance and Microsoft Advertising attribution.
- No `Windows Hello`
